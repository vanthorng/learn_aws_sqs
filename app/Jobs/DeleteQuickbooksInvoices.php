...
class DeleteQuickbooksInvoices implements ShouldQueue
{
    ...
    public function handle(QuickBooksClient $client): void
    {
        ...
        foreach ($operation->items as $index => $item) {
            try {
                $invoice = $client->deleteInvoice($connection, $item->qbo_invoice_id);
                InvoiceRecord::query()->where('qbo_invoice_id', $item->qbo_invoice_id)
                    ...
                    ->update(['qbo_deleted_at' => now(), 'qbo_delete_error' => null]);
                $item->update([
                    'status' => 'completed', 'doc_number' => $invoice['DocNumber'] ?? null,
                    'customer' => $invoice['CustomerRef']['name'] ?? null, 'error_summary' => null,
                ]);
                ...
                InvoiceRecord::query()->where('qbo_invoice_id', $item->qbo_invoice_id)
                    ->whereHas('invoiceImport', fn ($query) => $query->where('team_id', $operation->team_id))
                    ->update(['qbo_delete_error' => $exception->getMessage()]);
